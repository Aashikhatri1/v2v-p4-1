    ...
    for sentence in sentences:
        ...
        if sentence not in processed_sentences:
            ...
            if 'have a great day' in sentence.lower():
                ...
                print('stop playing', stop_playing) 
                play_audio_from_text(sentence)
                print('stop playing', stop_playing) 
...
async def pplx_streaming(messages, chat_history1, query, output_filename):
    global chat_history
    ...
    chat_history_str = str(chat_history1)
        ...
        if 'choices' in response:
            ...
            for part in parts[:-1]:  # Exclude the last part for now
                ...
                if part and len(part.split()) > 1:
                    ...
                    try:
                        task11 =  asyncio.create_task(handle_gpt_response(part + '.'))
            ...
            if last_part:
                ...
                if sentence_end_pattern.search(last_part):
                    ...
                    try:
                        task12 =  asyncio.create_task(handle_gpt_response(last_part))
    ...
    if last_part:
        ...
        # await handle_gpt_response(last_part)
        try:
            task13 =  asyncio.create_task(handle_gpt_response(last_part))
    ...
    if content.strip():        
        ...
        chat_history1.append({"role": "user", "content": query})
        chat_history1.append({"role": "assistant", "content": content})
...
async def ask_question(chat_user_info, chat_history, query, ask_question_prompt, output_filename):
    ...
    try:
        task3 =  asyncio.create_task(pplx_streaming(messages, chat_history, query, output_filename))
        
        # print('chat_history async:', chat_history)
    except Exception as e:
        print(e)
        return
    # chat_history = task2.result()
...
async def rooms_availability_final_answer(rooms_data, info, chat_user_info, chat_history, query, prompt4, output_filename):
    ...
    try:
        task4 =  asyncio.create_task(pplx_streaming(messages, chat_history, query, output_filename))
...
async def final_answer(info, chat_history, query, prompt3, output_filename):
    ...
    # return chat_history
    try:
        task5 =  asyncio.create_task(pplx_streaming(messages, chat_history, query, output_filename))
...
async def handle_transcript(transcript):
    ...
    global task1, task2, task3, task4, task5, task6, task7, task8, task9, task10, task11, task12
    global stop_playing
    global previous_query
    ...
    if transcript and not transcript.isspace():
        ...
        if task2 and not task2.done():
            print('task2 stopped')
            task2.cancel()
        if task3 and not task3.done():
            print('task3 stopped')
            task3.cancel()
        if task4 and not task4.done():
            print('task4 stopped')
            task4.cancel()
        if task5 and not task5.done():
            print('task5 stopped')
            task5.cancel()
        if task6 and not task6.done():
            print('task6 stopped')
            task6.cancel()
        if task7 and not task7.done():
            print('task7 stopped')
            task7.cancel()
        if task8 and not task8.done():
            print('task8 stopped')
            task8.cancel()
        if task9 and not task9.done():
            print('task9 stopped')
            task9.cancel()
        if task10 and not task10.done():
            print('task10 stopped')
            task10.cancel()
        if task11 and not task11.done():
            print('task11 stopped')
            task11.cancel()
        if task12 and not task12.done():
            print('task12 stopped')
            task12.cancel()
...
async def process_transcription_data(name):
    global task1, task2, task5, task6, task7, task8, task9, task10
    ...
    try: 
        ...
        if question_type == "FAQ":
            ...
            if ContextGiven == "No":
                ...
                try:
                    task1 =  asyncio.create_task(final_answer('', chat_history, query, prompt3, output_filename))
                    print(task1)
                    # chat_history = task1.result()
                    print('chat_history async:', chat_history)
                except Exception as e:
                    print(e)
                ...
                # chat_history =  asyncio.create_task(final_answer(info, chat_history, query, prompt3, output_filename))
                try:
                    task2 =  asyncio.create_task(final_answer(info, chat_history, query, prompt3, output_filename))
                    print(task2)
                    # print('chat_history async:', chat_history)
                except Exception as e:
                    print(e)
                ...
                await asyncio.sleep(0)
                # chat_history = await task2.result()
                set_or_add_value_by_name(name, 'done')
                await asyncio.sleep(0)
        # elif question_type == "DB":
            ...
            if info != []:
                    ...
                    if 'Information Required From Client' in info_item:
                        ...
                        if value is not None and value != 'NA':
                            ...
                            if chat_user_info == {}:
                                ...
                                try:
                                    task5 =  asyncio.create_task(ask_question(chat_user_info, chat_history, query, ask_question_prompt, output_filename))
                                    print(task5)
                                    # print('chat_history async:', chat_history)
                                except Exception as e:
                                    print(e)
                                ...
                                # chat_history= await ask_question(chat_user_info, chat_history, query, ask_question_prompt, output_filename)
                                try:
                                    task6 =  asyncio.create_task(ask_question(chat_user_info, chat_history, query, ask_question_prompt, output_filename))
                                ...
                                try:
                                    task7 =  asyncio.create_task(rooms_availability_final_answer(filtered_rooms_data, info, chat_user_info, chat_history, query, prompt4, output_filename))
                            ...
                            try:
                                task8 =  asyncio.create_task(rooms_availability_final_answer("", info, chat_user_info, chat_history, query, prompt4, output_filename))
                        ...
                        # chat_history = await rooms_availability_final_answer("", info, chat_user_info, chat_history, query, prompt4, output_filename)
                        try:
                            task9 =  asyncio.create_task(rooms_availability_final_answer("", info, chat_user_info, chat_history, query, prompt4, output_filename))
                ...
                # chat_history = await rooms_availability_final_answer("", info, chat_user_info, chat_history, query, prompt4, output_filename)
                try:
                    task10 =  asyncio.create_task(rooms_availability_final_answer("", info, chat_user_info, chat_history, query, prompt4, output_filename))
