...
#######################################################################################################
# interrupted = False  # This flag indicates if the current processing was interrupted

# async def handle_transcript(transcript):
#     global transcription_buffer, interrupted
#     if transcript and not transcript.isspace():
#         transcription_buffer.append(transcript)
#         if interrupted:
#             # If processing was interrupted, we immediately start processing the accumulated transcripts
#             interrupted = False  # Reset the flag
#             new_data_event.set()
#         elif not new_data_event.is_set():
#             # If not interrupted and no processing is scheduled, schedule it
#             new_data_event.set()
...
#         if transcription_buffer:
#             data_to_process = " ".join(transcription_buffer)
#             transcription_buffer = []  # Clear buffer for future transcripts

#             # Start processing the accumulated transcripts
#             try:
#                 await process_transcription_data(data_to_process)
#             except asyncio.CancelledError:
#                 # If processing was cancelled because a new transcript came in,
#                 # mark it as interrupted and ensure the data is not lost
#                 interrupted = True
#                 transcription_buffer.insert(0, data_to_process)  # Re-add the unprocessed data to the beginning of the buffer
#                 continue  # Immediately try processing the accumulated data again

#######################################################################################################
# transcription_buffer = []  # Buffer for accumulating transcripts
# current_processing_task = None  # Reference to the current processing task
# new_data_event = asyncio.Event()  # Event to signal new data arrival

# async def handle_transcript(transcript):
#     global transcription_buffer, current_processing_task, new_data_event
#     if transcript and not transcript.isspace():
#         transcription_buffer.append(transcript)
#         if current_processing_task and not current_processing_task.done():
#             current_processing_task.cancel()  # Cancel the current task
#         new_data_event.set()  # Signal to start processing
...
#             # Start or restart processing the accumulated transcripts
#             current_processing_task = asyncio.create_task(process_transcription_data(data_to_process))
...
async def process_transcriptions():
    ...
    while True:
        ...
        # Do not clear the transcription_buffer here.
        # Only copy the data to process and clear it AFTER deciding to start processing.
        data_to_process = " ".join(transcription_buffer)
        ...
        # Clear the buffer right before starting a new processing task,
        # so any new incoming transcripts will be added to a fresh buffer.
        transcription_buffer = []  
        ...
        try:
            ...
            # If processing is cancelled, the current data is already being handled,
            # and any new data will be appended to the buffer.
            print("Processing was cancelled due to new transcript.")
...
async def process_transcription_data(query):
